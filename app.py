...
lang_dict = {
    "English": {
        ...
        "guide_0": "**1. [ 0% ] The Fallacy of the SI Constant (The Geometric Illusion)**<br>Modern astrophysics relies on the SI constant ($c$), calibrated within Earth's average gravity. At 0%, you are viewing the universe through this 'bent ruler'. In **Case 1**, the three vectors fail to intersect. In **Case 2** (zoomed in), standard physics is completely blind, producing a massive 'Probability Arc' spanning hundreds of square degrees.",
        "guide_50": "**2. [ 0% ➔ 99% ] The K-PROTOCOL Calibration (Changing the Lens)**<br>As you move the slider, you mathematically strip away Earth's localized gravitational distortion ($S_{loc}$) and apply the Absolute Kinematic Speed of Light ($c_k$). Guided by absolute geometric necessity, the disjointed vectors bend and align. Notice the right-side 2D graphs—microsecond residuals plummet exponentially toward true zero.",
        "guide_100": "**3. [ 100% ] The Zero-Error Singularity (Deterministic Collapse)**<br>At exactly 100%, vectors strike absolute astronomical coordinates without a single millimeter of deviation. Crucially, **Case 2 proves that with the K-PROTOCOL, we do not need a third detector.** The specific distortion ratio between two points uniquely identifies the source in 3D space. The universe is deterministic.",
    ...
    "한국어": {
        ...
        "guide_0": "**1. [ 0% ] SI 단위계의 기하학적 왜곡 (착시 현상의 실태)**<br>현대 천체물리학은 지구 평균 중력에 오염된 SI 광속 상수($c$)를 절대 잣대로 오인하여 사용합니다. **Case 1(검출기 3개)**에서는 세 선이 빗나가며 오차를 만듭니다. 가장 심각한 것은 **Case 2(검출기 2개)**입니다. 현대 물리학은 검출기가 2개일 때 삼각측량이 불가능하다며 수백 평방도의 거대한 '확률의 붉은 안개(Arc)'만 허공에 뿌려놓습니다.",
        "guide_50": "**2. [ 0% ➔ 99% ] K-PROTOCOL 보정 메커니즘 (절대 시공간으로의 전환)**<br>슬라이더를 움직이면, 각 관측소의 국소적 중력 렌즈($S_{loc}$)가 걷히고 우주의 절대 운동 광속($c_k$)이 적용됩니다. 빗나가던 선들이 자석처럼 정렬하며, 줌인된 Case 2의 거대 안개는 맹렬하게 한 점을 향해 수축(Collapse)합니다. 2D 그래프에서는 학계가 노이즈로 치부했던 시간 잔차가 0을 향해 수직 낙하합니다.",
        "guide_100": "**3. [ 100% ] 무결점 특이점의 도달 (결정론적 우주의 완성)**<br>100% 도달 시, 모든 관측 벡터는 1mm의 오차도 없이 천문 절대 좌표(RA, Dec)에 정확히 꽂힙니다. 이 시뮬레이션의 핵심은 **Case 2**입니다. **각 관측소의 중력 왜곡($S_{loc}$)만 기하학적으로 완벽히 보정해 낸다면, 제3의 검출기가 없더라도 단 2개의 신호만으로 우주 공간을 완벽히 역추적할 수 있음**을 확증합니다.",
        "success": "🎯 **[Q.E.D. 증명 완료]** 검증(Case 1) 및 불가능으로 여겨졌던 발견(Case 2) 시나리오 모두 단 하나의 결정론적 좌표로 수렴하였습니다."
    }
}
...
def render_case(header, slider_label, is_case1, key_suffix):
    ...
    with c1:
        ...
        for k in show_list:
            ...
            # 선분 및 끝점 마커 추가
            fig3d.add_trace(go.Scatter3d(x=[starts[k][0], end[0]], y=[starts[k][1], end[1]], z=[starts[k][2], end[2]], mode='lines+markers', line=dict(color=colors[k], width=4), marker=dict(size=[0, marker_size], color=colors[k], symbol='cross'), name=names[k]))
            # 시작점(관측소) 텍스트 명칭 라벨링
    ...
    with c2:
        ...
        for k in show_list:
            ...
            fig2d.add_trace(go.Scatter(x=x_vals, y=log_errors, mode='lines', name=names[k], line=dict(color=colors[k], width=3)))
